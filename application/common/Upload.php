...
class Upload
{
    ...
    protected $config = [
         ...
         * 文件保存格式
         */
        'savekey'   => '/uploads/{year}{mon}{day}/{filename}', //原文件名
         ...
         * 可上传的文件类型
         */
        'mimetype'  => 'jpg,png,bmp,jpeg,gif,zip,rar,xls,xlsx,wav,mp4,mp3,pdf,docx,mkv',
        /**
         * 是否支持批量上传
         */
        'multiple'  => false,
     ...
     * @throws UploadException
     */
    public function ossUpload($savekey = null)
    {
        if (empty($this->file)) {
            throw new UploadException(__('No file upload or server upload limit exceeded'));
        }
        ...
        $oss = new OSS;
        $result = $oss->upload($fileName,$destDir.$fileName);
        $category = request()->post('category');
        $category = array_key_exists($category, config('site.attachmentcategory') ?? []) ? $category : '';
        $auth = Auth::instance();
        $params = array(
