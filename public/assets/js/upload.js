            ...
            api: {
                ...
                upload: function (element, onUploadSuccess, onUploadError, onUploadComplete) {
                    ...
                    $(element, Upload.config.container).each(function () {
                        ...
                        var middle = $(this).data("middle") ? $(this).data("middle") : "";
                        ...
                        var chunking = false, chunkSize = 2097152, timeout = 600000;

                        //最大可上传文件大小
                        maxsize = typeof maxsize !== "undefined" ? maxsize : '20M';
                        //文件类型
                        mimetype = typeof mimetype !== "undefined" ? mimetype : 'jpg,png,bmp,jpeg,gif,zip,rar,xls,xlsx,wav,mp4,mp3,pdf,docx,mkv';
                        //请求的表单参数
                        ...
                        Upload.list[id] = new Dropzone(this, $.extend({
                            ...
                            dictDefaultMessage: "将文件放在指定位置上传",
                            dictFallbackMessage: "您的浏览器不支持拖放文件上传",
                            dictFallbackText: "请使用下面的回退表像以前一样上传您的文件",
                            dictFileTooBig: "文件太大超过限制"+maxFilesize,
                            dictInvalidFileType: "不能上传这种类型的文件",
                            dictResponseError: "服务器响应",
                            dictCancelUpload: "取消上传",
                            dictUploadCanceled: "上传已取消",
                            dictCancelUploadConfirmation: "确实要取消此上载吗？",
                            dictRemoveFile: "删除文件",
                            dictMaxFilesExceeded: "超过上传个数",
                            init: function () {
                        ...
                        if (preview_id && input_id) {
                            ...
                            $(document.body).on("keyup change", "#" + input_id, function (e) {
                                ...
                                $.each(inputArr, function (i, j) {
                                    ...
                                    var url = j;
                                    var fullurl = location.origin + middle + url;

                                    var suffix = /[\.]?([a-zA-Z0-9]+)$/.exec(j);
                                    suffix = suffix ? suffix[1] : 'file';
                                    if (suffix == 'docx') {
                                        icon = location.origin + middle +'assets/img/docx.svg'
                                    } else if (suffix == 'xlsx') {
                                        icon = location.origin + middle +'assets/img/xlsx.svg'
                                    } else if (suffix == 'xls') {
                                        icon = location.origin + middle +'assets/img/xls.svg'
                                    } else if (suffix == 'txt') {
                                        icon = location.origin + middle +'assets/img/txt.svg'
                                    } else if (suffix == 'pdf') {
                                        icon = location.origin + middle +'assets/img/pdf.svg'
                                    } else if (suffix == 'zip') {
                                        icon = location.origin + middle +'assets/img/zip.svg'
                                    } else if (suffix == 'rar') {
                                        icon = location.origin + middle +'assets/img/rar.svg'
                                    } else if (suffix == 'mp3') {
                                        icon = location.origin + middle +'assets/img/mp3.svg'
                                    } else if (suffix == 'mp4') {
                                        icon = location.origin + middle +'assets/img/mp4.svg'
                                    } else if (suffix == 'mkv') {
                                        icon = location.origin + middle +'assets/img/mkv.svg'
                                    } else {
                                        icon = location.origin + middle +'' + url;
                                    }
                                   
                                    var value = (json && typeof json[i] !== 'undefined' ? json[i] : null);
                                    
                                    var html = '<li class="col-xs-3"><a href="'+ fullurl +'" data-url="'+ url +'" target="_blank" class="thumbnail"><img src="'+ icon +'" class="img-responsive"></a><a href="javascript:;" class="btn btn-danger btn-xs btn-trash"><i class="fa fa-trash"></i>删除</a></li>';
